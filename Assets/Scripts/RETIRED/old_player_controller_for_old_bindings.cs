...
public class player_controller : MonoBehaviour
{
    ...
    bool check_ground(Collision collision) {
        Debug.Log(Physics.Raycast(transform.position, transform.TransformDirection(Vector3.down), Mathf.Infinity));
            ...
            (collision.gameObject.tag == "floor" || collision.gameObject.tag == "jump_obj"))
        {
            ...
            Debug.DrawLine(gameObject.transform.position,
                new Vector3(gameObject.transform.position.x, gameObject.transform.position.y - rayDistance, gameObject.transform.position.z), Color.red);
            return true;
        }
        return false;
    ...
    void handle_jump(){
        if (Input.GetKeyDown(KeyCode.C) && (isGrounded || doubleJump >= 0))
        {
            rb.velocity = Vector3.up * jump_force;
            isGrounded = false;
            doubleJump -= 1;
        }
        else if (Input.GetKey(KeyCode.C) && !falling && !isGrounded)
        {
            Debug.Log("jumping");
        ...
        else if (Input.GetKeyUp(KeyCode.C) && !falling && !isGrounded)
        {
            falling = true;
            gravity_scale *= scale;
            rb.velocity = new Vector3(rb.velocity.x, 0.0f, rb.velocity.y);
